...
class GetSpatialGatingWeights(nn.Module):
    ...
    def __init__(self, in_channels, block_size, grid_size, dim_u, dim_v, input_proj_factor=2, dropout_rate=0.0,
                 use_bias=True):
        ...
        self.dense_u = nn.Linear(dim_u, dim_u, bias=use_bias)
        nn.init.normal_(self.dense_u.weight, std=2e-2)
        nn.init.ones_(self.dense_u.bias)
        self.dense_v = nn.Linear(dim_v, dim_v, bias=use_bias)
...
class CrossGatingBlock(nn.Module):
    ...
    def __init__(self, in_channels, features, block_size, grid_size, dim, dropout_rate=0.0, upsample_y=True,
                 use_bias=True):
